...
_menu_status() {
    local kernel_status proxy_status tun_status
    local current_sub_id current_sub_url
    local mixed_port http_port socks_port

    clashstatus >&/dev/null && kernel_status=$'\033[32m[ON]\033[0m' || kernel_status=$'\033[31m[OFF]\033[0m'

    local sub_id=$("$BIN_YQ" '.use // ""' "$CLASH_PROFILES_META" 2>/dev/null)
    if [ -n "$sub_id" ] && [ "$sub_id" != "null" ]; then
        current_sub_id="$sub_id"
        current_sub_url=$(_get_url_by_id "$sub_id" 2>/dev/null)
        [ ${#current_sub_url} -gt 35 ] && current_sub_url="${current_sub_url:0:32}..."
    else
        current_sub_id="-"
        current_sub_url="未设置"
    fi

    local tun_val=$("$BIN_YQ" '.tun.enable // false' "$CLASH_CONFIG_RUNTIME" 2>/dev/null)
    [ "$tun_val" = "true" ] && tun_status=$'\033[32m[ON]\033[0m' || tun_status=$'\033[31m[OFF]\033[0m'

    local proxy_val=$("$BIN_YQ" '._custom.system-proxy.enable // false' "$CLASH_CONFIG_MIXIN" 2>/dev/null)
    [ "$proxy_val" = "true" ] && proxy_status=$'\033[32m[ON]\033[0m' || proxy_status=$'\033[31m[OFF]\033[0m'

    mixed_port=$("$BIN_YQ" '.mixed-port // ""' "$CLASH_CONFIG_RUNTIME" 2>/dev/null)
    http_port=$("$BIN_YQ" '.port // ""' "$CLASH_CONFIG_RUNTIME" 2>/dev/null)
    socks_port=$("$BIN_YQ" '.socks-port // ""' "$CLASH_CONFIG_RUNTIME" 2>/dev/null)

    _menu_box "clashctl 控制面板"
    _menu_row "内核: $KERNEL_NAME    状态: $kernel_status"
    _menu_row "订阅: [$current_sub_id] $current_sub_url"
    _menu_row "Tun:  $tun_status    系统代理: $proxy_status"
    _menu_row "端口: mixed=${mixed_port:-无}  http=${http_port:-无}  socks=${socks_port:-无}"
    _menu_box_end
}

_menu_main_items() {
    printf "\n"
    printf "  [1] 🔌 开启代理         [2] 🔌 关闭代理\n"
    printf "  [3] 📡 订阅管理         [4] 🔧 Tun 模式\n"
    printf "  [5] 🌐 系统代理         [6] 🔑 Web 密钥\n"
    printf "  [7] 🖥️  Web 控制台       [8] 📋 查看日志\n"
    printf "  [9] ⬆️  升级内核         [10] ⚙️  Mixin 配置\n"
    printf "  [0] 🚪 退出\n"
}

_menu_main() {
    # 保存并替换 _error_quit，避免在菜单中 exec $SHELL
    local _saved_error_quit
    _saved_error_quit=$(declare -f _error_quit)
    eval "${_saved_error_quit/_error_quit/_orig_error_quit}"
    _error_quit() {
        [ $# -gt 0 ] && {
            local color=#f92f60
            local emoji=📢
    ...
    while true; do
        ...
        6)
            local secret
            _read_choice "输入新密钥（留空查看当前）:" secret
            if [ -n "$secret" ]; then
                clashsecret "$secret"
            else
                clashsecret
            ...
            ;;
        7) clashui; _pause ;;
        8) clashlog | tail -30; _pause ;;
        9) clashupgrade; _pause ;;
        10) _menu_mixin ;;
        0) break ;;
        *) _failcat "无效选项：$choice"; _pause ;;
        esac
    done

    # 恢复原始 _error_quit
    eval "${_saved_error_quit/_orig_error_quit/_error_quit}"
}
...
_menu_sub_items() {
    ...
    local sub_url
    if [ -n "$sub_id" ] && [ "$sub_id" != "null" ]; then
        sub_url=$(_get_url_by_id "$sub_id" 2>/dev/null)
        [ ${#sub_url} -gt 35 ] && sub_url="${sub_url:0:32}..."
    else
        sub_id="-"
        sub_url="未设置"
    fi
    ...
    printf "\n"
    _menu_box "订阅管理"
    _menu_row "当前: [$sub_id] $sub_url"
    _menu_box_end
    printf "\n"
    printf "  [1] ➕ 添加订阅         [2] ➖ 删除订阅\n"
    printf "  [3] 🔄 切换订阅         [4] ⬆️  更新订阅\n"
    printf "  [5] 📋 订阅列表         [6] 📜 订阅日志\n"
    printf "  [0] ⬅️  返回主菜单\n"
}
...
_menu_sub() {
    while true; do
        ...
        _menu_sub_items
        local choice
        _read_choice "请选择 [0-6]:" choice
        case "$choice" in
        1)
            local url
            _read_choice "请输入订阅链接:" url
            [ -n "$url" ] && _sub_add "$url"
            _pause
            ;;
        2)
        ...
        3)
            _sub_list
            local id
            _read_choice "请输入要使用的订阅 id:" id
            [ -n "$id" ] && _sub_use "$id"
            _pause
            ;;
        4)
            local id
            _read_choice "请输入要更新的订阅 id（留空更新当前）:" id
            _sub_update $id
            _pause
            ;;
        5) _sub_list; _pause ;;
        6) _sub_log | tail -20; _pause ;;
        0) break ;;
        *) _failcat "无效选项：$choice"; _pause ;;
        esac
    done
...
_menu_tun_items() {
    ...
    printf "\n"
    _menu_box "Tun 模式"
    _menu_row "状态: $tun_status"
    _menu_box_end
    printf "\n"
    printf "  [1] 🔛 开启 Tun         [2] 🔻 关闭 Tun\n"
    printf "  [0] ⬅️  返回主菜单\n"
}
...
_menu_tun() {
    while true; do
        ...
        _menu_tun_items
        local choice
        _read_choice "请选择 [0-2]:" choice
        case "$choice" in
        1) clashtun on; _pause ;;
        2) clashtun off; _pause ;;
        0) break ;;
        *) _failcat "无效选项：$choice"; _pause ;;
        esac
    done
...
_menu_proxy_items() {
    ...
    printf "\n"
    _menu_box "系统代理"
    _menu_row "状态: $proxy_status"
    _menu_box_end
    printf "\n"
    printf "  [1] 🔛 开启系统代理     [2] 🔻 关闭系统代理\n"
    printf "  [3] 📋 查看代理状态     [0] ⬅️  返回主菜单\n"
}
...
_menu_proxy() {
    while true; do
        ...
        _menu_proxy_items
        local choice
        _read_choice "请选择 [0-3]:" choice
        case "$choice" in
        1) clashproxy on; _pause ;;
        2) clashproxy off; _pause ;;
        3) clashproxy; _pause ;;
        0) break ;;
        *) _failcat "无效选项：$choice"; _pause ;;
        esac
...
_menu_mixin_items() {
    printf "\n"
    _menu_box "Mixin 配置"
    _menu_row "配置文件: $CLASH_CONFIG_MIXIN"
    _menu_box_end
    printf "\n"
    printf "  [1] 📄 查看 Mixin       [2] ✏️  编辑 Mixin\n"
    printf "  [3] 📋 原始订阅配置     [4] 🔄 运行时配置\n"
    printf "  [0] ⬅️  返回主菜单\n"
}
...
_menu_mixin() {
    while true; do
        ...
        _menu_mixin_items
        local choice
        _read_choice "请选择 [0-4]:" choice
        case "$choice" in
        1) clashmixin; _pause ;;
        2) clashmixin -e; _pause ;;
        3) clashmixin -c; _pause ;;
        4) clashmixin -r; _pause ;;
        0) break ;;
        *) _failcat "无效选项：$choice"; _pause ;;
        esac
    done
