...
_menu_box() {
    ...
    _color_log "$color" "$border_top"
    # 计算标题显示宽度（中文算 2 列，ASCII 算 1 列）
    local title_len=0
    local ch
    ...
    local pad_left=$(( (w - 2 - title_len) / 2 ))
    local pad_right=$(( w - 2 - title_len - pad_left ))
    [ $pad_left -lt 0 ] && pad_left=0
    [ $pad_right -lt 0 ] && pad_right=0
    local line="║"
    for ((i = 0; i < pad_left; i++)); do line+=" "; done
    line+="$title"
    for ((i = 0; i < pad_right; i++)); do line+=" "; done
    line+="║"
    _color_log "$color" "$line"
    _color_log "$color" "$border_mid"
}

_menu_box_end() {
    local color=${1:-#c8d6e5}
    local w=$_MENU_WIDTH
    local border="╚"
    local i
    for ((i = 0; i < w - 2; i++)); do border+="═"; done
    border+="╝"
    _color_log "$color" "$border"
}
...
_menu_row() {
    ...
    local pad=$(( w - 2 - text_len ))
    [ $pad -lt 0 ] && pad=0
    # 直接 printf 输出，保留 text 中的 ANSI 颜色码
    local hex="${color#\#}"
    local r=$((16#${hex:0:2}))
    local g=$((16#${hex:2:2}))
    local b=$((16#${hex:4:2}))
    printf "\033[38;2;${r};${g};${b}m║  %b" "$text"
